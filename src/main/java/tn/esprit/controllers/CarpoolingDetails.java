...
public class CarpoolingDetails {
    ...
    void updateNavigation(ActionEvent event) throws IOException {
        ...
        controller.initialize(carpoolingId); // Pass the carpoolingId to the initialize method
    ...
    public void initialize() {
        User loggedUser = SessionUser.getLoggedUser();
        System.out.println("useeeeeer"+loggedUser);
        if (checkLoginStatus(loggedUser)) {
            try {
                Carpooling carpooling = carpoolingService.getById(carpoolingId);
                System.out.println("carpoooooliiiing"+carpooling);
                if (carpooling != null && carpooling.getUserID() == loggedUser.getId()) {
